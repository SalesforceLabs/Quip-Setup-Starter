...
import checkIfPageExists from '@salesforce/apex/AccountPlanHandler.checkIfPageExists';
import checkIfFlowExists from '@salesforce/apex/Utils.checkIfFlowExists';
import checkTabPositioningOnPage from '@salesforce/apex/AccountPlanHandler.checkTabPositioningOnPage';
import checkQuipTemplate from '@salesforce/apex/Utils.checkQuipTemplate';
import deployAccountPlan from '@salesforce/apex/AccountPlanHandler.deployAccountPlan';
import deployFlow from '@salesforce/apex/AccountPlanHandler.deployFlow';
import getLastRun from '@salesforce/apex/AccountPlanHandler.getLastRun';
import storeRun from '@salesforce/apex/AccountPlanHandler.storeRun';

import HOME_IMAGE from '@salesforce/resourceUrl/accountPlan';
import TAB_SAMPLE_IMAGE from '@salesforce/resourceUrl/tabSample';

const DEFAULT_TEMPLATE = 'https://quip.com/wRaXA2fCE3fn';

export default class AccountPlanComponent extends LightningElement {
    ...
    get verificationSteps() {
        ...
        steps.push({
            ...
            description: `Quip uses this field to store the Account Plan URL for every ${this.objectNameLabel}`,
        ...
        if (this.recordPageCopy) {
            steps.push({
                text: `Create a copy of the Record Page and call it ${this.recordPageLabel} - with Quip Account Plan`,
            ...
            steps.push({
                text: `Create a custom tab called ${this.tabName} on ${this.recordPageLabel} - with Quip Account Plan`,
    ...
    get flowLabel() {
        return `Quip - ${this.objectName} - Create Account Plan`;
    }
    ...
    startEmbed() {
        deployAccountPlan({
            ...
            .catch(error => {
                console.error(error);
                this.showNotification('An Error Occurred', `Account Plan embedding failed: ${error.body.message}`, 'error');
                this.handleBack();
            });
    }
