...
import checkIfPageExists from '@salesforce/apex/OppNotesHandler.checkIfPageExists';
import checkIfFlowExists from '@salesforce/apex/Utils.checkIfFlowExists';
import checkTabPositioningOnPage from '@salesforce/apex/OppNotesHandler.checkTabPositioningOnPage';
import checkQuipTemplate from '@salesforce/apex/Utils.checkQuipTemplate';
import deployOppNotes from '@salesforce/apex/OppNotesHandler.deployOppNotes';
import deployFlow from '@salesforce/apex/OppNotesHandler.deployFlow';
import getLastRun from '@salesforce/apex/OppNotesHandler.getLastRun';
import storeRun from '@salesforce/apex/OppNotesHandler.storeRun';

import HOME_IMAGE from '@salesforce/resourceUrl/opportunityNotes';
import TAB_SAMPLE_IMAGE from '@salesforce/resourceUrl/tabSample';

const DEFAULT_TEMPLATE = 'https://quip.com/vIchAPxuW2Ad';

export default class OppNotesComponent extends LightningElement {
    ...
    get isTemplateTypeDefault() {
        return this.templateType === 'default';
    }

    get isTemplateTypeCustom() {
    ...
    get verificationSteps() {
        ...
        steps.push({
            ...
            description: `Quip uses this field to store the Opportunity Notes URL for every ${this.objectNameLabel}`,
        ...
        if (this.recordPageCopy) {
            steps.push({
                text: `Create a copy of the Record Page and call it ${this.recordPageLabel} - with Quip Opportunity Notes`,
            ...
            steps.push({
                text: `Create a custom tab called ${this.tabName} on ${this.recordPageLabel} - with Quip Opportunity Notes`,
        ...
        if (this.templateType === 'default') {
            steps.push({
                text: 'Create a copy of the Starter Template, so you can edit it later',
                description: 'You will find it in your Private folder at the end of the process',
                preflightCheck: this.preflightCheckResults.templateCheck,
                checkTrue: 'Starter template is ready to use',
                checkFalse: 'Starter template is not accessible'
            });
    ...
    get flowLabel() {
        return `Quip - ${this.objectName} - Create Opportunity Notes`;
    }
    ...
    startEmbed() {
        deployOppNotes({
            ...
            copyTemplate: this.templateType === 'default',
            ...
            .catch(error => {
                console.error(error);
                this.showNotification('An Error Occurred', `Opportunity Notes embedding failed: ${error.body.message}`, 'error');
                this.handleBack();
            });
    }
    ...
    handleStartAgain() {
        ...
        this.tabName = 'Opportunity Notes';
    ...
    storeFlowResult() {
        storeRun({
            ...
            starterTemplate: this.isTemplateTypeDefault,
            tabName: this.tabName,
